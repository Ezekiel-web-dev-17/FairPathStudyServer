...
/**
 * Generates a highly premium, modern, responsive HTML email template for password reset.
 */
...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Reset Your Password - FairPath Study</title>
  <style>
    body {
      font-family: 'Outfit', 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
      background-color: #f4f5f7;
...
<body>
  <div class="wrapper">
    <div class="container">
      ...
      <div class="content">
        <p>Hello,</p>
        <p>We received a request to reset the password for your FairPath Study account.</p>
        <p>To set a new password, please click the secure button below:</p>
        ...
        <div class="warning-box">
          <p><strong>Security Warning:</strong> This password reset link is valid for the next <strong>15 minutes</strong>. If you did not request a password reset, please ignore this email or contact support.</p>
        </div>
 ...
 * In development, it logs the reset link to the console/logger.
...
export const sendResetPasswordEmail = async (email: string, token: string): Promise<void> => {
  ...
  logger.info(`[Email Service] Password reset email initiated for ${email.replace(/^(\w{3}).*(@.+)$/, "$1****$2")}`);
  ...
  if (RESEND_API_KEY && RESEND_API_KEY !== 're_placeholder_key' && NODE_ENV !== 'test') {
    try {
      const { data, error } = await resend.emails.send({
        ...
        to: [email],
        subject: 'Reset Your Password - FairPath Study',
        html: htmlContent,
      });

      if (error) {
        logger.error(`Resend reset password email failed for ${email}:`, error);
      } else {
        logger.info(`Resend reset password email delivered: ${data?.id}`);
      }
    } catch (err) {
      logger.error(`Resend reset password email error for ${email}:`, err);
    }
  } else {
